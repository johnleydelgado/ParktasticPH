...
const useGetBookingLogs = (onlyGetParkingSpaceData: boolean) => {
  ...
  const fetchBookingLogs = async () => {
    let allData: listOfBookingProps[] = [];
    const parkingLotsSnapshot = await firestore()
    ...
    for (const doc of parkingLotsSnapshot.docs) {
      try {
        ...
        if (!bookingsObj || bookingLogsObj.timeOutLog) {
          status = 'Available';
          dataToPush = {...doc.data()};
          allData.push({
            ...dataToPush,
        ...
        if (isWithinRange(-1, 1, true)) {
          ...
          dataToPush = {...doc.data(), bookingId: bookingLogsObj.bookingId};
  ...
  // Function to fetch parking space data
  const fetchParkingSpaceData = async () => {
    ...
    const parkingSpaceArray = parkingSpaceSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }));

    return parkingSpaceArray[0] as ParkingSpacesProps;
  };

  const {
    data: bookingListLogs,
    isLoading: isBookingLogsLoading,
    isError: isBookingLogsError,
    refetch: refetchBookingLogs,
    isRefetching: IsRefetchBookingLogs,
  } = useQuery(['bookingLogs', updateTrigger], fetchBookingLogs, {
    enabled: !onlyGetParkingSpaceData,
  });

  const {
    data: parkingSpaceData,
    isLoading: isParkingSpaceLoading,
    isError: isParkingSpaceError,
    refetch: refetchParkingSpaceData,
    isRefetching: IsRefetchParkingSpaceData,
  } = useQuery(['parkingSpaceData', updateTrigger], fetchParkingSpaceData, {
    enabled: onlyGetParkingSpaceData,
  });
  ...
  return {
    bookingListLogs,
    parkingSpaceData,
    loading: isBookingLogsLoading || isParkingSpaceLoading,
    error: isBookingLogsError || isParkingSpaceError,
    refetch: onlyGetParkingSpaceData
      ? refetchParkingSpaceData
      : refetchBookingLogs,
    isRefetching: onlyGetParkingSpaceData
      ? IsRefetchParkingSpaceData
      : IsRefetchBookingLogs,
  };
};
